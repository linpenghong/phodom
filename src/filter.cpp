...
Filter::Filter(std::shared_ptr<Parameter> parameter_, std::shared_ptr<ImuBuffer> imuBuffer_) : bodyState(parameter_), feature_tracker(100), cameraPoseBuf(200),	triangulation(this) {
...
//    stateAugment();
    ...
    pruneCameraPoses();//need to check
...
//    std::cout << std::endl;
    ...
    while(cameraPoseBuf.size() > maxTrackNum + 1) {
