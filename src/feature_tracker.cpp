...
FeatureTracker::feature_track_list FeatureTracker::processImage(feature_track_list& previous_tracks, cv::Mat& image) {
    ...
    FeatureFrame frame_features = FeatureFrame::detectFeatures(detector_, extractor_, working_image);
    ...
    if (previous_frame_features_.keypoints().size() == 0) {
        ...
        for (std::size_t i = 0; i < frame_features.keypoints().size(); ++i) {
            ...
            current_features.back()->pushFeaturePosition(keypoint.pt.x/scale_factor, keypoint.pt.y/scale_factor);
        }
    ...
    for (std::size_t i = 0; i < matches.size(); ++i) {
        ...
        if (match.distance < previous_feature_matched[train_idx]) {
            if (!std::isinf(previous_feature_matched[train_idx])) {
                ...
                previous_tracks[train_idx]->popFeaturePosition();
            ...
            current_tracks[query_idx]->pushFeaturePosition(
...
void FeatureTracker::markOutOfViewFeatures(std::vector<double>& feature_matched, feature_track_list& feature_tracks) const {
...
void FeatureTracker::createNewFeatureTracks(std::vector<bool> &feature_matched,
        feature_track_list &feature_tracks, const FeatureFrame& frame_features, double scale_factor) const {
    for (std::size_t i = 0; i < feature_tracks.size(); ++i) {
        if (!feature_matched[i]) {
            ...
            feature_track->pushFeaturePosition(current_keypoint.pt.x/scale_factor, current_keypoint.pt.y/scale_factor);
